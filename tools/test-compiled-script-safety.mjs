...
await check('Compiled registrations respect filtering levels', async () => {
    ...
    const context = vm.createContext({
        ...
        sharedScriptletContext: async () => ({ schema: 1, payload: {}, nativeExclusions: new Map(), stock: [] }),
        stockScriptletSources: async () => ({}),
        prepareNativeStockScriptlets: () => [],
        removeNativeStockScriptlets: async () => [],
        restoreNativeStockScriptlets: async () => {},
        SCRIPTLET_WARNINGS_KEY: 'scriptletExceptions.warnings',
        localWrite: async () => {},
        recordLoggerEvent: () => {},
