...
Chrome không chạy được regex mà engine RE2 của nó từ chối, ví dụ khi chương trình sau biên dịch vượt giới hạn bộ nhớ của trình duyệt. Trên Chromium, quá trình build kiểm tra mọi rule regex đóng gói bằng chính Chrome: rule nào Chrome chấp nhận được đóng gói vào ruleset tĩnh của từng danh sách, còn các rule khác, kể cả ngoại lệ, bị bỏ ra và được tính vào số bộ lọc danh sách không chuyển đổi được. Vì vậy thay đổi lựa chọn danh sách không còn bị từ chối vì lý do này. Rule regex của danh sách nhập vào và Bộ lọc của tôi dùng chung một giới hạn riêng 1.000 rule với chặn nghiêm ngặt và được kiểm tra khi cài: khi giới hạn đó đầy, rule regex chặn của danh sách nhập vào bị bỏ từ cuối danh sách kèm cảnh báo, còn một ngoại lệ hoặc một rule của Bộ lọc của tôi không cài được thì tiện ích giữ nguyên cấu hình và quy tắc trước đó. Bản Firefox giữ cách xử lý cũ cho danh sách đóng gói. Mục Chẩn đoán hiển thị cả hai giới hạn. Xem [giới hạn regex native và cơ chế khôi phục](MV3-PARITY-IMPLEMENTATION-2026-09-06.md) và [Power Runtime](POWER-RUNTIME.md).
