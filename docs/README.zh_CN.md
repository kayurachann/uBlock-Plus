...
> [!IMPORTANT]
> **发布状态：** v1.0.0 是供手动侧载的预发布版本，不会自动更新。uBlock Plus+ 不是 uBlock Origin 或 uBO Lite 的官方版本，也未得到 Raymond Hill 的认可。Chrome MV3 并未提供原 MV2 扩展可用的全部拦截原语。侧载可以避开 Chrome 应用商店的分发政策，但**不会**消除 DNR 配额、Service Worker 生命周期规则或浏览器安全边界。请参阅[如实说明的兼容性矩阵](FEATURE-MATRIX.md)。
...
[阅读架构文档](ARCHITECTURE.md) · [探索 Power Runtime](POWER-RUNTIME.md) · [查看威胁模型](THREAT-MODEL.md) · [了解隐私](PRIVACY.md)
...
| --- | --- | --- |
| DNR 网络拦截、外观过滤、内置 scriptlet、自定义/导入列表、Filter Store、元素选择器/移除器、可感知上下文的按主机弹出窗口策略，以及备份/恢复 | 实时请求日志、过程式过滤器、导入的弹出窗口过滤器执行、动态防火墙语义、响应头操作和重定向行为仅能部分等同于 MV2 | 托管式企业适配器，以及独立安装的开源原生伴侣程序；均须经过 RFC、用户同意和安全审查 |

普通公开 MV3 扩展 API 不提供任意响应正文重写、等效的 DNS/CNAME 可见性或精确的按响应大小拦截。部分 MV2 过滤语法无法转换；在假定功能等效之前，请查阅功能矩阵。导入网络列表的编译现会记录稳定的拒绝原因和源代码行号；在控制面板中以更丰富的形式呈现该报告仍属于路线图工作。
...
uBlock Plus+ 是基于 [uBlock Origin](https://github.com/gorhill/uBlock) 及其 MV3/uBO Lite 实现的衍生作品。项目保留版权、源文件头、作者历史和第三方署名。请参阅 [NOTICE.md](../NOTICE.md)。
