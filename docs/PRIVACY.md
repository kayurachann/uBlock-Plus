...
When strict blocking sends a blocked page to the extension's warning page (Chromium builds), the page asks the extension's service worker which address was blocked, so that it can show it and offer **Proceed**. The service worker keeps:

- the full address of the last blocked page of each tab, with the time it was blocked and the list that blocked it, until the warning page has read it. These records are kept in memory and in the browser's session storage (`storage.session`), so that a restarted service worker can still answer, for at most 64 tabs. A record is deleted as soon as the warning page has read it, or when its tab closes, and all of them are gone when the browser exits or the extension is reloaded or updated. The extension opens its session storage to its own content scripts, so a compromised web page process could read these addresses while they wait there; a stored record dated in the future is ignored;
- with the `webRequest` permission, the address of each tab's recent top-level requests, in memory only, for at most 64 requests: a redirect by the server hides the blocked address from the redirect event, so the service worker takes it from the request's previous address. An entry is deleted when its request reaches the warning page, when the permission is removed, or when newer requests replace it, and is lost when the service worker stops;
- the address where the last top-level navigation of each tab started, for at most 64 tabs, in memory only. It is used only when no exact source is available, and the page then says that the address is approximate and does not offer **Don't warn me again about this site**, since that address may belong to a site which redirected to the blocked one. It is deleted when the tab closes and is lost when the service worker stops.

The warning page itself keeps the address in its own URL fragment and history state, as the page already did with the address in its URL before. The browser stores it with the tab's history like any page address. Hostnames for which you select **Proceed** are kept in session storage until the browser exits; hostnames for which you select **Don't warn me again about this site** are kept in local storage. Neither is included in backups.

None of this is sent anywhere. Troubleshooting reports include only which address source is in use (for example `rule-match`) and regex-rule counts, never an address.
...
- `declarativeNetRequestFeedback`: native matched-rule diagnostics when Chrome and the installation type support it. For the logger, matches are observed only while the opt-in logger captures a tab, within its 512-record lifecycle; Developer mode keeps no separate matched-rule buffer. Strict blocking also uses it (Chromium, unpacked installations only): while a strict-blocking redirect is installed, the optional `webRequest` permission is not granted and the low-memory profile is not in effect, the service worker listens to Chrome's matched-rule events to learn the exact blocked address. Chrome delivers an event for every matched rule; the extension keeps only top-level strict-blocking matches, as described in [Strict blocking](#strict-blocking). Nothing is transmitted by the project.
- Optional `webRequest` in the standard edition: requested only when the user clicks **Start capture** in the logger, or **Grant network-observation permission** in **Dashboard → Diagnostics**. The logger observes requests only for explicitly captured tabs, and its observers are removed when capture stops. While the permission is granted, strict blocking also listens to top-level requests and redirects and keeps only the addresses that end on its own warning page (see [Strict blocking](#strict-blocking)); this replaces the matched-rule events above and costs less. It does not grant synchronous request blocking. Refusing permission leaves the other available diagnostic sources usable, and the warning page then shows an exact address only on unpacked installations.
- Required `webRequest` and `webRequestBlocking` in the separate [Experimental WebRequest edition](EXPERIMENTAL-WEBREQUEST.md): evaluate saved firewall block rules synchronously when Chrome grants access. The supplementary listener keeps bounded tab context in memory, respects Off and skips uncertain contexts. Strict blocking uses the same top-level redirect listener as the standard edition does once the optional permission is granted. Request logging remains opt-in. The launcher uses a separate local Chrome profile and does not edit enterprise policy, registry entries or the existing personal profile. It installs no native service and sends no telemetry. The stable manifest public key identifies the variant; it is not a signing secret.
...
- `webNavigation`: correlate newly created navigation targets and enumerate opener frames for the Smart Popup Blocker. Candidate URLs and trusted-gesture targets exist in memory for at most 30 seconds and 5 seconds respectively; service-worker recovery checkpoints retain only origin-level URLs, while the bounded session diagnostic log retains at most 100 hostname-only decisions. Paths, queries, fragments and credentials are not persisted. Strict blocking also keeps the start address of each tab's last top-level navigation in memory, as its approximate fallback ([Strict blocking](#strict-blocking)); it is not persisted either.
