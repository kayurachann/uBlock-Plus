...
3. Compiler tạo static/dynamic DNR, cosmetic data và tham chiếu tới packaged scriptlet. Với Chromium và Edge (bước 2 của [lộ trình](MV3-LIMITS-RESEARCH-2026-09-24.md#lộ-trình-11-bước)):
   - **Cổng RE2:** mọi regex sẽ nằm trong static ruleset phải qua `re2-portable.js` (tập con RE2 an toàn cho mọi Chrome từ 130, bắt buộc) rồi `isRegexSupported` của Chrome thật. `regex-verdicts.mjs` mở Chrome headless qua pipe, trong profile tạm mới và với một extension thử tạm, rồi chỉ đóng cây process do chính nó mở và xóa cả hai thư mục tạm. Build phát hành (`-Version`) dừng nếu không có Chrome (`regexVerdicts=required`); build development không có Chrome thì dùng verdict đã cache; regex không có verdict chỉ qua tập con portable, và log ghi rõ gói chưa được Chrome kiểm tra. Regex Chrome không chạy được bị đếm là `unsupported-regex-memory`/`unsupported-regex-syntax`.
   - Regex hợp lệ được nối vào cuối `main/<id>.json` của list theo thứ tự `rulesets.json`, miễn tổng của mọi static ruleset không vượt 1.000; phần vượt ở lại `rulesets/regex/<id>.json` (đường dynamic). `rulesets/regex-details.json` ghi số regex static, phiên bản Chrome đã kiểm và digest. Hiện là 174 regex static với list mặc định, 473 với mọi list.
   - `rulesets/strictblock/<id>.json` là redirect `extensionPath` tới `/strictblock.html` ở priority 29, chỉ `main_frame`, giữ nguyên `requestDomains`, `urlFilter` hoặc `regexFilter` của filter. Chỉ filter vốn là regex mới cần regex rule (111 trong 1.106 rule ở list mặc định).
   - Firefox và Safari giữ regex stock dynamic và strict-block `regexSubstitution`.
...
- đặt regex stock vào pool static thay vì pool dùng chung. Với list mặc định, pool static dùng 174/1.000 (473 với mọi list); pool dùng chung chỉ còn 111 regex strict-block của list stock (123), để khoảng 889 (877) cho Bộ lọc của tôi, list nhập thêm và rule DNR developer. `regex-capacity.js` đếm pool dùng chung theo chủ sở hữu, dựa trên dải ID trong `dnr-namespaces.js`;
...
- **Strict blocking:**
  - Session rule ID 1–999.999 thuộc kế hoạch strict-block (`strictblock-rules.js`). Kế hoạch gồm redirect của list stock đang bật (priority 29), redirect cho `$doc`/`$document`/`$all` của Bộ lọc của tôi và list nhập thêm, và allow cho site được loại trừ. Redirect của người dùng nằm ở P+1 trên block dynamic của chính filter (P); block đó không bao giờ bị sửa và là dự phòng khi redirect không được cài.
  - Kế hoạch được dựng lại trong hàng đợi DNR chung, bằng một lần `updateSessionRules`: sau mỗi lần cập nhật rule người dùng, khi đổi site loại trừ hay chế độ strict, khi quyền thay đổi, và khi worker khởi động mà kế hoạch đã lưu (`storage.session['strictBlock.plan']`) không còn khớp. Lần cập nhật thất bại giữ kế hoạch cũ.
  - Không có quyền truy cập mọi trang web thì không có redirect nào, vì redirect thiếu quyền host che block bên dưới và để trang tải. Redirect của người dùng còn cần một nguồn URL chính xác, và chỉ có trên Chromium.
  - Hết ngân sách session hoặc regex thì rule strict-block stock bị bỏ trước và được báo.
  - `strictblock-tracker.js` ghi địa chỉ bị chặn của mỗi tab từ `webRequest` (`onBeforeRedirect`, và `onBeforeRequest` cho bước tới `strictblock.html` sau server redirect) hoặc `onRuleMatchedDebug` (không dùng ở low-memory profile), và lúc bắt đầu điều hướng từ `webNavigation.onBeforeNavigate` làm nguồn gần đúng. Tối đa 64 tab, trong bộ nhớ và `storage.session`, xóa khi trang đã đọc hoặc khi đóng tab. Listener được đăng ký đồng bộ ở top level; chi phí của chúng nằm trong [POWER-RUNTIME.md](POWER-RUNTIME.md#chi-phí-của-strict-block-cho-service-worker).
  - `strictblock.html` hỏi địa chỉ bằng `getStrictBlockDetails`, message mà worker chỉ trả lời cho chính trang đó trong frame trên cùng của một tab. Trang rồi giữ địa chỉ trong fragment và `history.state` của entry lịch sử, và chỉ mở URL `http(s)`.
...
User-imported HTTPS list ──────> validator/compiler ──> dynamic DNR
                                             └───────> typed popup data
Build machine ──stock regex──> Chrome headless (profile tạm, isRegexSupported) ──> verdict
Page request ───────────────────────────────> Chrome DNR
Blocked document ──> Chrome DNR redirect ──> strictblock.html <── URL bị chặn ── tracker trong service worker
                                                                  (webRequest / onRuleMatchedDebug / webNavigation)
