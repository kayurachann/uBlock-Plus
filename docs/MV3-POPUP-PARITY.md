...
This does not emulate unsupported MV2 behavior. Unsupported popup conditions and missing context continue to defer. The matcher evaluates exceptions before blocks. An exhausted budget still defers while an exception could apply, or while an unevaluated important block could outrank a matching exception. Otherwise the remaining block rules are skipped and the Smart/Strict policy still decides. A popup URL longer than 8 KB is judged by its origin only. Exceptions anchored (`||`) to a hostname which cannot start in that origin are ruled out, and block rules which only the dropped path could match are skipped in the same way. An exception which could still match the dropped path keeps deferring. For an unrelated hostname, the packaged default lists contain no such exception; `rus-0` does, so with that list enabled a popup padded past 8 KB still defers. Deferred allows remain conservative guards; stock popunder omission and complete-corpus suppression remain intact. DNR quotas, worker lifetime, browser restrictions and packaged-script requirements still apply. The popup does not invent full request counts or present limited DNR diagnostics as a complete network logger. See [the feature matrix](FEATURE-MATRIX.md) for engine-level limitations.
