...
This is an implemented firewall supplement, **not a port of the complete uBlock Origin network engine or an unlimited MV3 mode**. Imported, personal and packaged static network filters still use the existing DNR compilers, which report most of the filters they cannot convert with a reason code. DNR rule and regex limits, the firewall editor's limits, response-body restrictions and unsupported inline-script filtering still apply. Compare against [the full uBO compatibility matrix](FEATURE-MATRIX.md). Which of these limits this package could lift, and at what cost, is analysed in the [MV3 limits research of 24 September 2026](MV3-LIMITS-RESEARCH-2026-09-24.md) (Vietnamese).
...
**Re-check on Google Chrome 153.0.8010.53, 24 September 2026.** Small test extensions were run in fresh temporary profiles, which were deleted afterwards:

- The switch still grants `webRequestBlocking` to an unpacked MV3 extension. It also granted `dns` to a test extension whose manifest declares it; this package does not declare `dns`. Without the switch, neither is granted. Cancel, redirects (including `main_frame`), and adding or removing response headers took effect.
- Chromium implements the switch as a test hook, [`IsAllowlistedForTest`](https://github.com/chromium/chromium/blob/153.0.8010.53/extensions/common/features/simple_feature.cc#L735-L743). It skips every allowlist and install-location check for that ID, not only the one for `webRequestBlocking`. The switch is on the [list of flags](https://github.com/chromium/chromium/blob/153.0.8010.53/chrome/browser/ui/startup/bad_flags_prompt.cc#L119-L121) that show the unsupported-flag warning bar, and Chrome can remove it in any release.
- The extension is still not policy-installed. [Promise responses are ignored](https://github.com/chromium/chromium/blob/153.0.8010.53/extensions/renderer/api/web_request_natives.cc#L92-L102): requests whose listener returned a Promise (immediate or delayed cancel, `async` function, redirect) all reached the server, while the same decisions returned synchronously took effect.
- `chrome.dns.resolve()` returns one IP address and never a CNAME chain, so the grant does not enable CNAME uncloaking.
- DNR block and redirect decisions still win over the listener. DNR rules conditioned on response headers act at `onHeadersReceived`, after the request was sent.
- Relaunching the same dedicated profile with the switch across browser restarts, and a cold browser start, were not re-tested.
...
The research measured what such an engine would cost here, on one machine (AMD Ryzen 5 5600H, 8 GB RAM) and one Chrome build. Restoring uBO's `static-net-filtering.js` from an IndexedDB snapshot took 60–75 ms per real worker start with the default lists (138,922 filters) and 75–95 ms with 23 lists (203,598 filters). With realistic pauses between page loads (5 or 35 seconds), up to 44 of a test page's roughly 150 requests, always including its `main_frame` request, passed the listener before the engine was ready; only DNR filtered them. Building the engine synchronously at the worker's top level made Chrome hold the requests instead (0 of 32 passed) at a delay of 100–115 ms, measured with the default lists only. Matching took 30–150 µs per request over the first few hundred requests after a start and 6–8 µs once warm; worker memory grew by 9–16 MiB. DNR stays the always-on baseline even so: Chrome can be started without the switch, user state such as filtering modes and enabled lists is still read asynchronously, and a cold browser start was not measured.
