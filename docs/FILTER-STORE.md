...
Khi người dùng bật entry, runtime dùng `installation.sourceURL` làm nguồn chính và compile như imported list. Nguồn pinned có SHA-256 được kiểm lại trong compiler; chỉ sau khi compile và cập nhật DNR thành công UI mới ghi nhận digest đã kích hoạt. Digest khớp không tự nâng entry lên `verified`. Mỗi import từ Filter Store có ngân sách tối đa 5 MiB/list, tối đa 32 lần fetch/include và ngân sách batch 15 MiB. Phần chia của ngân sách batch chỉ giới hạn lần tải đầu tiên của list; các lần refresh sau dùng giới hạn 5 MiB/list, nên list lớn dần sau khi cài vẫn cập nhật được. Nguồn pinned là bất biến nên không được tải lại theo `Expires`; khi cần tải lại qua mạng (khôi phục backup, bật lại, nâng compiler), chỉ số byte sau giải nén và SHA-256 quyết định, không dùng `Content-Length` (header này đếm byte gzip). List không pinned hết hạn được tải lại trong lần compile kế tiếp; nếu tải hoặc compile bản mới thất bại, hoặc bản mới làm generation không kích hoạt được quá 1 giờ, bản compile đã kích hoạt gần nhất vẫn được dùng, nguyên nhân được lưu trong `refreshError` của list và lần thử sau lùi dần từ 1 giờ tới tối đa 1 ngày. Khi tắt, metadata imported được giữ để người dùng có thể bật lại, còn compiled cache được giải phóng theo chính sách hiện hành. Filter Store không gửi danh sách site người dùng truy cập cho maintainer catalog.
