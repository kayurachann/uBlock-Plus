...
3. **Keep incomplete cosmetic results out of the page and cache.** If a dictionary cannot be read, it could contain an exception to another list. The lookup now fails open without caching a partial result, so another navigation can retry. Off scopes come from the worker's effective-mode session copy whenever it exists. That copy is the user's modes with the administrator's noFiltering Off scopes added and its '-host'/'-*' entries removed, and the worker writes it before the stored modes. The user's stored modes are used only when the session copy is missing, for example early in startup; there, an absent key means the defaults, which have no Off scope. An unreadable stored record with no session copy, a malformed mode record, or a mode change during dictionary loading cancels the lookup and leaves the page unchanged. A profile which has never stored modes uses the defaults, which have no Off scope, so site-specific hiding still applies on a fresh install. An already trusted page skips dictionary reads. The session-cache write is not awaited before hiding; the cache-prune notification is sent after that write completes.
