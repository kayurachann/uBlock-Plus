...
Chrome cannot run a regex which its RE2 engine rejects, for example because the compiled program exceeds the browser's memory limit. On Chromium, the build checks every packaged regex rule with Chrome: the rules Chrome accepts are packaged in the lists' own static rulesets, and the others, exceptions included, are left out and counted among the filters the list could not convert. Choosing lists is therefore no longer rejected for this reason. Regex rules of imported lists and My filters share a separate limit of 1,000 with strict blocking and are checked when they are installed: when that limit is full, imported regex block rules are left out from the end, with a warning, while an exception or a My filters rule which cannot be installed keeps the previous configuration and rules. Firefox builds keep the previous behaviour for packaged lists. Diagnostics shows both limits. See the [native regex limit and recovery details](docs/MV3-PARITY-IMPLEMENTATION-2026-09-06.md) and [Power Runtime](docs/POWER-RUNTIME.md).
